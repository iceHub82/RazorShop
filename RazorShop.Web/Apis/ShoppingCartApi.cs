...
{
    public static void ShoppingCartApi(this WebApplication app)
    {
        app.MapGet("/shoppingcart", async (HttpContext http, RazorShopDbContext db, IMemoryCache cache) =>
        {
            var cart = await GetShoppingCart(http, db);
            var items = await GetShoppingCartItems(cart.Id, db)!;
    ...
    private static async Task<Cart> GetShoppingCart(HttpContext http, RazorShopDbContext db)
    {
        Cart? cart;
    ...
    private static async Task<List<CartItem>>? GetShoppingCartItems(int cartId, RazorShopDbContext db)
    {
        return await db.CartItems!.Where(c => c.CartId == cartId && !c.Deleted).Include(c => c.Product).ToListAsync();
    }

    private static ShoppingCartVm? GetShoppingCartViewModel(List<CartItem> cartItems, IMemoryCache cache)
    {
        ...
        return new ShoppingCartVm {
            ShoppingCartQuantity = cartItems.Sum(c => c.Quantity),
            ShoppingCartItems = cartItems.Select(item => new ShoppingCartItemVm {
                Id = item.Id,
            ...
            ShoppingCartTotal = $"{cartItems.Sum(c => c.Product!.Price * c.Quantity):#.00} kr"
        };
    }
}